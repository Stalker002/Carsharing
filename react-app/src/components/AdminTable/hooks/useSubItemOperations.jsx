...
} from "../../../redux/actions/clientDocuments";
import { getClientDocument, updateClient } from "../../../redux/actions/clients";
import {
  fieldsFines,
  fieldsInsurances,
  fieldsMaintenances,
  fieldsPayments,
  fieldsClientDocuments,
} from "../configs";
...
export const useSubItemOperations = (activeTab, editingItem, subData) => {
      ...
      dispatch(
        openModal({
          ...
          onConfirm: async () => {
            ...
            switch (subType) {
              case "insurance":
                result = await dispatch(deleteInsurance(id));
                break;
              case "maintenance":
                result = await dispatch(deleteMaintenance(id));
                break;
              case "fine":
                result = await dispatch(deleteFine(id));
                break;
              case "payment":
                result = await dispatch(deletePayment(id));
                break;
              case "document":
                result = await dispatch(deleteClientDocument(id));
                break;
              default:
                break;
            }
  ...
  const handleSubAddSave = useCallback(
    async (data) => {
      ...
      switch (subType) {
        ...
        case "document":
          ...
          docFormData.append("ClientId", subData.clientProfile.id);
          docFormData.append("Type", data.type);
          docFormData.append("Number", data.number);
          docFormData.append("IssueDate", data.issueDate);
          docFormData.append("ExpiryDate", data.expiryDate);

          if (data.licenseCategory) {
            docFormData.append("LicenseCategory", data.licenseCategory);
          }

          if (data.file) {
            const fileToSend =
              data.file instanceof FileList ? data.file[0] : data.file;
            docFormData.append("File", fileToSend);
          } else {
            throw new Error("Выберите файл документа");
          }
          result = await dispatch(createClientDocument(docFormData));
          dispatch(getClientDocument(subData.clientProfile.id))
          break;
        default:
          break;
      }
  ...
  const handleSubUpdateSave = useCallback(
    async (e) => {
      e.preventDefault();
      ...
      switch (subType) {
        ...
        case "payment":
          result = await dispatch(
            updatePayment(finalData.id, {
              ...finalData,
              billId: Number(editingItem.id),
              sum: Number(finalData.sum),
              date: new Date(finalData.date).toISOString(),
            })
          );
          break;
        case "document":
          result = await dispatch(
            updateClientDocument(finalData.id, {
              ...finalData,
              clientId: subData.clientProfile.id,
            })
          );
          break;
        default:
  ...
  const handleClientProfileSave = useCallback(
    async (formData) => {
      if (!subData.clientProfile?.id) return;
      const result = await dispatch(
        updateClient(subData.clientProfile.id, {
          userId: Number(editingItem.id),
          ...subData.clientProfile,
          ...formData,
        })
      );
      if (result.success) {
  ...
  const subFields = useMemo(() => {
    switch (subType) {
      ...
      case "fine":
        return fieldsFines.filter((f) => f.name !== "tripId");
      case "payment":
        return fieldsPayments.filter((f) => f.name !== "billId");
      case "document":
        return fieldsClientDocuments.filter((f) => f.name !== "clientId");
      default:
        return [];
    }
  }, [subType]);
